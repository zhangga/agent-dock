...
在 `Backup` 页点击 `Check updates` 可以生成只读更新报告。对 GitHub 来源的 skill，AgentDock 会读取远端 repository tree，把本地安装目录的文件 hash 和远端 skill 目录比较，然后标记为 `up_to_date` 或 `update_available`；如果无法定位远端目录或无法可靠比较，则保守标记为 `unknown`。未安装但有来源的 skill 会标记为 `not_installed`；缺少来源的 skill 会标记为 `needs_source`。这个检查不会自动安装、更新或改写备份文件。
